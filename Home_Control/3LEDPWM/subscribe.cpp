...
#define TOPIC       "ChrisCprag/feeds/homecontrol.pwmslider"
...
int msgarrvd(void *context, char *topicName, int topicLen, MQTTClient_message *message) {
    ...
    printf(" %f",atof(payloadptr));
    led.setDutyCycle((float)(atof(payloadptr)));
...
int main(int argc, char* argv[]) {
    ...
    led.setPeriod(20000000);
    led.run();
    led.setDutyCycle(0.0f);
    led.setPolarity(PWM::ACTIVE_LOW);
    do {
        ch = getchar();
    } while(ch!='Q' && ch != 'q');
